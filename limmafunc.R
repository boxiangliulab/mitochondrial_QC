limmafunc<-function(paircomp,colnam1,path0,thresh,bioinfo){
  ord <- c("MHC/W",unique(as.vector(colnam1))[!unique(as.vector(colnam1)) %in% c("MHC/W")])
  colnam2<-colnam1[order(match(colnam1,ord))]
  rownam<-paircomp[,1]
  paircomp0<-paircomp[,seq(2,dim(paircomp)[2])]
  paircomp0<-(apply(paircomp0,2,as.numeric))
  ...
  # Ordinary fit
  fit <- lmFit(paircomp0,design)
  fit2 <- eBayes(fit)
  table1<-topTable(fit2,coef=2,n=Inf)
  table1[,"Accession"]<-rownames(table1)
  setwd(file.path(root0,path0))
  table0<-merge(bioinfo,table1,by="Accession")
  ...
  # Volcano plot
  table1$updown<-'NO'
  table1$updown[table1$logFC>thresh & table1$adj.P.Val<0.05]<-'Up'
  table1$updown[table1$logFC<(-thresh) & table1$adj.P.Val<0.05] <-'Down'
  ...
  print(ggplot(data=table1,aes(x=logFC,y=-log10(adj.P.Val),col=updown,label=Accession))+ geom_vline(xintercept=c(-thresh, thresh), col="purple") +
          theme_minimal()+
