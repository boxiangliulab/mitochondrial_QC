limmafunc_p<-function(paircomp,colnam1,path0,thresh,bioinfo){
  ...
  table1<-topTable(fit2,coef=2,n=Inf)  #table in limma
  ...
  table1$updown[table1$logFC>thresh & table1$P.Value<0.05]<-'Up'  #set up regulation
  table1$updown[table1$logFC<(-thresh) & table1$P.Value<0.05] <-'Down'  #set down regulation
  
  table0$updown<-'NO'
  table0$updown[table0$logFC>thresh & table0$P.Value<0.05]<-'Up'
  table0$updown[table0$logFC<(-thresh) & table0$P.Value<0.05] <-'Down'
  ...
  table2<-tablesig
  table3<-table2[(table2[,"P.Value"]<0.05 & abs(table2[,"logFC"])>thresh),]
  ...
  print(ggplot(data=table0,aes(x=logFC,y=-log10(P.Value),col=updown,label=table0$'Gene Symbol'))+ geom_vline(xintercept=c(-thresh, thresh), col="purple") +
