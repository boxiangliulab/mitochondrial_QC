library(readr)
library(dplyr)
root0<-'/Users/rushi/Desktop/NUS/Y4S2/FYP'
if(!dir.exists(root0)){
  dir.create(root0)
}

batch1_renamed <- read_csv("/Users/rushi/Desktop/NUS/Y4S2/FYP/Yi's data/batch1_renamed.csv")
batch2_renamed <- read_csv("/Users/rushi/Desktop/NUS/Y4S2/FYP/Yi's data/batch2_renamed.csv")
...
final2024 <- data.frame(final1_8) %>% select(-starts_with("FMR"))
originalcolname = colnames(final1_8)
pcalabel<-str_replace_all(originalcolname,"_batch\\d*","")
pcalabell<-paste0("    ",pcalabel)
originalcolname2024 = colnames(final2024)
pcalabel2024<-str_replace_all(originalcolname2024,"_batch\\d*","")
pcalabell2024<-paste0("    ",pcalabel2024)
colnames(final2024)<-pcalabel2024
groups = c(rep('MHC/W',3),rep('USP10-PA OE',3),rep('USP10 sh',3),
           'FMR1 RNAi-2','RIN OE','RIN RNAi',rep('FMR1 OE',3),rep('FMR1 RNAi-2',3),
           rep('RIN OE',3),rep('RIN RNAi',3),'MHC/W')
groups2024 = c(rep('MHC/W',3),rep('USP10-PA OE',3),rep('USP10 sh',3),
           'RIN OE','RIN RNAi',
           rep('RIN OE',3),rep('RIN RNAi',3),'MHC/W')
source("/Users/rushi/Desktop/NUS/Y4S2/FYP/Yi's data/pcafunc.R")
pcacombat1_8<-pcafunc(final1_8)  #use prcomp to do PCA
pca2024 <- pcafunc(final2024)
library(factoextra)
if (!dir.exists(file.path(root0,'PCA_2024'))){
  dir.create(file.path(root0,'PCA_2024'))
}
setwd(file.path(root0,'PCA_2024'))
...
id_list = read.csv('/Users/rushi/Desktop/NUS/Y4S2/FYP/id_validation_table_115027.csv')
chr_data = read.csv('/Users/rushi/Desktop/NUS/Y4S2/FYP/FlyBase_Fields_download.csv')
...
write.csv(comp_gene_info2,file=file.path(root0,"Complete_gene_info_CG_Chr2024.csv"),row.names = FALSE)
...
#remove controls: 2024
clean2024 <- subset(final2024, select = -c(RIN_OE_01, RIN_RNAi_01, MHC_W_04))  #updated final
colnames(clean2024)[1]<-'Accession'
clean2024.2 = as.data.frame(clean2024)
clean2024.2[,'CG_number'] = comp_gene_info2[,'ANNOTATION_SYMBOL'][match(clean2024[,'Accession'], comp_gene_info2[,'Accession'])]  #add CG number
clean2024.2[, 'Description'] = comp_gene_info2[, 'Gene Symbol'][match(clean2024[,'Accession'], comp_gene_info2[,'Accession'])]  #add gene symbol
write.csv(clean2024.2,file=file.path(root0,path1,"BatchRemoveWithoutControl 2024.csv"),row.names = FALSE)
...
###### PCA 2024 no FMR1 OE RANi #####
originalcolname2<-colnames(clean2024)
pcalabel2<-str_replace_all(colnames(clean2024),"_batch\\d*","")
#pcalabel2<-paste0("    ",pcalabel2)
colnames(clean2024)<-pcalabel2
groups2024<-c(rep('Control',3),rep('USP10 OE',3),rep('USP10 sh',3),
           rep('RIN OE',3),rep('RIN RNAi',3))
groups2024 = factor(
  groups2024,
  levels = c(rep('Control',3),rep('USP10 OE',3),rep('USP10 sh',3),
...
pcacombat2024<-pcafunc(clean2024)  #use prcomp to do PCA
if (!dir.exists(file.path(root0,'PCA_2024'))){
  dir.create(file.path(root0,'PCA_2024'))
}
setwd(file.path(root0,'PCA_2024'))
tiff("Combat PCA 2024.tiff",units="in", width=11, height=10,res=250)  #safe as high resolution picture
options(ggrepel.max.overlaps=Inf)
fviz_pca_ind(pcacombat2024,         #plot PCA
             col.ind = groups2024, # color by groups
             palette = c("#00AFBB","#FC4E07", 'orange','blue','#336600'), #"grey50","grey50",
             addEllipses = TRUE, # Concentration ellipses
             ellipse.type = "confidence",
             legend.title = "Groups",
             pointsize = 5,
...
library(limma)
library(ggrepel)
source("/Users/rushi/Desktop/NUS/Y4S2/FYP/Yi's data/limmafunc2.R")
...
setwd("/Users/rushi/Desktop/NUS/Y4S2/FYP/Volcano plot")
...
setwd("/Users/rushi/Desktop/NUS/Y4S2/FYP/Dual volcano plot")
volcano.files <- list.files(pattern="^.*\\.csv")
for (i in 1:length(volcano.files)) {
  file = read.csv(paste0("/Users/rushi/Desktop/NUS/Y4S2/FYP/R_code/Dual volcano plot/", volcano.files[i]))
  ...
  if (i==1) {
    ...
    file$labels = ifelse(file$Gene.Symbol %in% c('Usp10','ref(2)P','Clbn','Fmr1', 'dZnf598'),'purple',FALSE)
...
for (i in 1:length(volcano.files)) {
  for (j in 1:length(volcano.files[-i])) {
    volcano1 = read.csv(paste0("/Users/rushi/Desktop/NUS/Y4S2/FYP/Dual volcano plot/", volcano.files[i]))
    volcano1[, "log10(adj.P.Val)"] = -(log10(volcano1$adj.P.Val))
    volcano2 = read.csv(paste0("/Users/rushi/Desktop/NUS/Y4S2/FYP/Dual volcano plot/", volcano.files[-i][j]))
    ...
    green.volcano$color_group <- 'Mitochondrial genes (adj. p < 0.05)'
    volcano$color_group <- volcano$updown
    tiff(paste0(name, '.tiff'), units="in", width=11, height=6,res=200)
    print(ggplot(data=volcano,aes(x=logFC,y=`log10(adj.P.Val)`,col=color_group)) +
            xlim(-2.5,2.5)+
            theme_minimal()+             
            scale_colour_manual(name="Gene Expression",values=mycolors)+
            geom_point(size = 1)+
            geom_point(data = green.volcano, aes(x=logFC,y=`log10(adj.P.Val)`),size=1.5)+
            ...
            geom_vline(xintercept = c(-0.585, 0.585), col="orange", size=0.2) +
            geom_vline(xintercept = 0, col = "black") +
            geom_hline(yintercept = 0, col = "black") +
            geom_text_repel(data=purple.volcano, aes(label = Gene.Symbol),
                            box.padding = unit(0.9, "lines"),point.padding = unit(0.5, "lines"),hjust = 1,col = 'purple', size = 5)+
...
tiff('test.tiff', units="in", width=11, height=6,res=200)
print(ggplot(data=volcano, aes(x=logFC, y=`log10(adj.P.Val)`)) +
  geom_text_repel(data=volcano[volcano$labels == 'purple', ],
...
library(forcats)
setwd('/Users/rushi/Desktop/NUS/Y4S2/FYP/GSEA/GSEA 1-8/GSEA results final')
gseafile = list.files(pattern="^.*\\.csv")
fmr1oe.file = read.csv(paste0('/Users/rushi/Desktop/NUS/Y4S2/FYP/GSEA/GSEA 1-8/GSEA results final/', gseafile[1]))
...
fmr1rnai.file = read.csv(paste0('/Users/rushi/Desktop/NUS/Y4S2/FYP/GSEA/GSEA 1-8/GSEA results final/', gseafile[2]))
...
rinoe.file = read.csv(paste0('/Users/rushi/Desktop/NUS/Y4S2/FYP/GSEA/GSEA 1-8/GSEA results final/', gseafile[3]))
...
rinrnai.file = read.csv(paste0('/Users/rushi/Desktop/NUS/Y4S2/FYP/GSEA/GSEA 1-8/GSEA results final/', gseafile[4]))
...
usp10sh.file = read.csv(paste0('/Users/rushi/Desktop/NUS/Y4S2/FYP/GSEA/GSEA 1-8/GSEA results final/', gseafile[6]))
...
usp10oe.file = read.csv(paste0('/Users/rushi/Desktop/NUS/Y4S2/FYP/GSEA/GSEA 1-8/GSEA results final/', gseafile[5]))
...
for (i in 1:length(plots)){
  ...
  print(ggplot(usp10oe, aes(x = NES, y = fct_reorder(pathways, -FDR.q.value))) + 
          xlim(1,2.5)+
...
rpl.list = list()
rps.list = list()
# rpl.df = data.frame(Genotype = rep(1,2), logFC = rep(1,2))
x.axis = rep(0,6)

#### RpL ####
for (i in 1:length(volcano.files)) {
  rp.file = read.csv(paste0("/Users/rushi/Desktop/FYP/R_code/Dual volcano plot/", volcano.files[i]))
  rpl.proteins = rp.file$Gene.Symbol[startsWith(rp.file$Gene.Symbol, 'RpL') & !is.na(rp.file$Gene.Symbol)]
  rpl.fc = rp.file$logFC[startsWith(rp.file$Gene.Symbol, 'RpL') & !is.na(rp.file$Gene.Symbol)]
  rpl.list[[i]] = rpl.fc
  name = unlist(strsplit(volcano.files[i], split='.', fixed=TRUE))[1]
  x.axis[i] = name
  # rpl.df$Genotype = append(rpl.df$Genotype, rep(name, length(rpl.fc)))
  # rpl.df$logFC = append(rpl.df$logFC, rpl.fc)
}
par(
  mar = c(8, 5, 4, 2)  # Increase the margins
)
boxplot(rpl.list, main = 'Box plot of log2 fold changes of RpL proteins', names = x.axis, las = 2, ylab = 'log2 FC',
        cex.axis=0.7)
rpl.df = data.frame(RpL.proteins = rpl.proteins, logFC = rpl.fc)

#### RpS ####
for (i in 1:length(volcano.files)) {
  rp.file = read.csv(paste0("/Users/rushi/Desktop/FYP/R_code/Dual volcano plot/", volcano.files[i]))
  rps.proteins = rp.file$Gene.Symbol[startsWith(rp.file$Gene.Symbol, 'RpS') & !is.na(rp.file$Gene.Symbol)]
  rps.fc = rp.file$logFC[startsWith(rp.file$Gene.Symbol, 'RpS') & !is.na(rp.file$Gene.Symbol)]
  rps.list[[i]] = rps.fc
  name = unlist(strsplit(volcano.files[i], split='.', fixed=TRUE))[1]
  x.axis[i] = name
}
...
boxplot(rps.list, main = 'Box plot of log2 fold changes of RpS proteins', names = x.axis, las = 2, ylab = 'log2 FC',
        cex.axis=0.7)
# write.csv(rpl.df, file = file.path())
...
fbgn_mapping <- read.delim("~/Desktop/NUS/Y4S2/FYP/Dual volcano plot/fbgn_NAseq_Uniprot_fb_2024_02.tsv", header=FALSE, comment.char="#")
colnames(fbgn_mapping)[3] = "FBgn_ID"
colnames(fbgn_mapping)[4] = "nucleotide_accession"
colnames(fbgn_mapping)[6] = "UniprotKB/Swiss-Prot/TrEMBL_accession"

# Read the gene summary file
gene_summary <- read.delim("~/Desktop/NUS/Y4S2/FYP/Dual volcano plot/best_gene_summary_fb_2024_02.tsv", header=FALSE, comment.char="#")
colnames(gene_summary) <- c("FBgn_ID", "Gene_Symbol", "Summary_Source", "Summary")

# Function to get top 50 genes with highest absolute logFC values
get_top_50_genes <- function(df) {
  df_ordered <- df[order(abs(df$logFC), decreasing = TRUE), ]
  top_50_genes <- df_ordered[1:50, c("Accession", "Gene.Symbol", "logFC", "adj.P.Val")]
  return(top_50_genes)
}

# Initialize list to store the top 50 genes data frames
top_genes_list <- list()

# Loop through each file, read the data, and store the top 50 genes data frame
volcano.files <- list.files(pattern="^.*\\.csv")
for (file in volcano.files) {
  df <- read.csv(file)
  top_genes <- get_top_50_genes(df)
  
  # Map Accession to FBGN using fbgn_mapping
  top_genes_mapped <- top_genes %>%
    merge(fbgn_mapping, by.x = "Accession", by.y = "UniprotKB/Swiss-Prot/TrEMBL_accession") %>% 
    #left_join(fbgn_mapping, by = c("Accession" = "UniprotKB/Swiss-Prot/TrEMBL_accession"))
    dplyr::select(Accession, Gene.Symbol, logFC, adj.P.Val, FBgn_ID) %>%
    mutate(updown = ifelse(logFC > 0, "up", "down")) %>%
    unique()
  print("gene mapped")
  
  # Merge with gene summary using FBGN
  top_genes_with_summary <- top_genes_mapped %>%
    left_join(gene_summary, by = c("FBgn_ID" = "FBgn_ID"))
  print("summary joined")
  
  top_genes_list[[file]] <- top_genes_with_summary
}

# Assign names to the list elements for easier reference
names(top_genes_list) <- volcano.files

# Print the results
for (name in names(top_genes_list)) {
  cat("Top 50 genes from:", name, "\n")
  print(top_genes_list[[name]])
  cat("\n")
}

# Optionally, you can save each data frame to a new CSV file
for (name in names(top_genes_list)) {
  write.csv(top_genes_list[[name]], paste0("top_50_genes_with_summary_", basename(name)), row.names = FALSE)
}
